...
DOMAIN = "discogs_wantlist_watcher"
    ...
    def my_service(call: ServiceCall) -> None:
        ...
        for offer in good_offers:
            ...
            send_notification(
                ...
                msg=f'media condition: {offer["media_condition"]}, sleeve condition: {offer["sleeve_condition"]}; price is only {offer["price"]} (max-price: {parse_price(offer["wantlist_item"])})\n marketplace stats: {get_price_stats(item.id, url=item.url)}\n tracklist: {item.tracklist]}',
                url=item.url,
                device=device)
